    ...
    chatForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (data.success && data.reply) {
          ...
          if (badgePill) {
            if (data.isFallback) {
              badgePill.textContent = 'DETERMINISTIC INTEL';
              if (data.debugReason) badgePill.title = `Gemini fallback reason: ${data.debugReason}`;
            } else {
              const modelDisplay = (data.modelUsed || 'AI SYNTHESIS').toUpperCase();
              badgePill.textContent = modelDisplay;
              badgePill.title = `Synthesized in real-time via Google ${data.modelUsed}`;
            }
          }
